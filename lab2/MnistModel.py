...
class MyModel:
    def __init__(self) -> None:
        with open(MF.save_path, "rb") as f:
            self.weightlist, self.biaslist, = pickle.load(f)

    def __call__(self, figure):
        num = 0
        pos = -1
        for is_num in range(10):
            pred = figure @ self.weightlist[is_num] + self.biaslist[is_num]
            if pred > pos :
                num = is_num
                pos = pred
        return num
