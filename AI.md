...
- **最大似然框架**
  ...
  - 在训练集上最大化对数似然
    - 代入可得
    - $$min\frac{1}{n}\sum_{i∈[n]}{}log[1+e^{-y_i(w^Tx_i+b)}]$$
    - 称为**交叉熵损失** (Cross Entropy Loss) ，有时也直接称为 Logistic/Log Loss
  ...
  - _**SUMMARY:**_
    - model
      - $f(x)=w^Tx+b$
      - $x∈R^d,w∈R^d,b∈R$
      - $σ(f(x))=\frac{1}{1+e^{f(x)}}$ 为转换为正类的概率
      - $1-σ(f(x))$ 为转换为负类的概率
    - 最大似然估计
      - $max_{w,b}\sum_{i∈[n]}log[p(y_i|x_i;w,b)]=max_{w,b}\sum_{i∈[n]}log[σ(y_i·f(x_i))]$
    - 最小交叉熵损失
      - $min_{w,b}\frac{1}{n}\sum_{i∈[n]}log[1+e^{-y_i(w^Tx_i+b)}]$
- 多分类问题
  - K>2
    - one or rest: K个二分类
    - 代价过于高
  - Softmax 回归
    - $p(y=k|x)=\frac{e^{f_k(x)}}{\sum_{j∈[K]}e^{f_j(x)}}$
    - 归一化
      - 使用最大对数似然估计参数
      - $max_{θ_k}\frac{1}{n}\sum_{i∈[n]}log[p(y_i|x_i)]$
      - 等价于交叉熵损失
      - $min_{w,b}\frac{1}{n}\sum_{i∈[n]}log[\frac{\sum_{j∈[K]}e^{f_j(x_i)}}{e^{f_k(x_i)}}]=min_{w,b}\frac{1}{n}\sum_{i∈[n]}({log[\sum_{j∈[K]}e^{f_j(x_i)}]}-{f_{y_i}(x_i)})$
  - **正则化**
    - 实际中我们一般在损失函数后加一项一起优化
    - $min_{w,b}\frac{1}{n}\sum_{i∈[n]}({log[\sum_{j∈[K]}e^{f_j(x_i)}]}-{f_{y_i}(x_i)})+λ·R(f)$
    - λ·R(f)称为正则化项 (regularization term)，用于惩罚过于复杂的模型
    - λ是个超参数 (hyperparameter)，预先指定，不随参数优化
    - 防止过拟合 (overfitting)，即测试误差远远高于训练误差
    - 过拟合原因
      - 某几个特征维度 𝑗 支配 (dominate) 了预测，即这些维度的权重 𝑤_𝑗 过大
        - 解决方法：L2 正则化 $𝑅(f)=||x||^2_2=w^Tw=\sum_{j∈[d]}w_j^2$ 会放大较大的权重，用于惩罚少数过大的权重维度，使权重分配更平均
      - 输入数据中存在大量没用的特征维度，但仍然赋予了它们非零的权重
    - 解决方法：L1 正则化 $𝑅(f)=||x||_1=\sum_{j∈[d]}|w_j|$ 作用：鼓励稀疏的 𝑤，即 𝑤 中大部分维度为零，仅有少数维度非零 （具体原因超出本课程范围）
...
- **决策树**
  - 决策树是一种树形结构，包含一个根节点、若干个内部节点和若干个叶节点。其中：
    • 根节点包含样本全集
    • 每个非叶节点对应于一个（特征）属性测试；其包含的样本集合根据属
    性测试的结果被划分到子节点中
    • 每个叶节点对应于决策结果（取多数类）
  - **最优划分属性**
    - 可以找到某一个属性𝑀，根据属性𝑀的不同取值对当前节点包含的样本集合进行划分后，每个子集都尽量只包含同类的标签
  - 纯度度量与划分准则
    - 信息熵
      - 对于离散型随机变量 𝑋，服从概率分布 𝑝(𝑥)，其信息熵定义为：
      - $$H(x)=\sum_xp(x)log_{2}\frac{1}{p(x)}$$
      - 当且仅当𝑝(𝑥)为一个“确定分布”时（即存在一个确定事件概率为1，其他事件概率为0），熵为零
      - 所有事件等概率时，不确定性最大
      - ![12](https://pic.imgdb.cn/item/642e8895a682492fcce58c4f.jpg)
    - 准则1：信息增益
      - 用信息熵度量样本集合𝐷的标签纯度：
      - ![111](https://pic.imgdb.cn/item/642e897fa682492fcce6e00c.jpg)
      - 属性𝐴对集合𝐷的信息增益：
      - ![123](https://pic.imgdb.cn/item/642e89dba682492fcce77f1f.jpg)
    - 准则2：增益率
